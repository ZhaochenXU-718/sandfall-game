...
- 运行时只加载一张 `512×256` RGBA 图集，包含 E01～E04 的 `12` 枚正式尺寸白色 Alpha 子图。
...
- `CocosVfxController` 初始化时一次性创建 `31` 个 Sprite 节点，游戏过程中不创建或销毁粒子节点。
- 默认上限为粒子 `24`、光环 `4`、拖尾 `3`，总活跃数不超过 `31`。
...
| 游戏事件 | 图集元素 | 反馈规则 |
| --- | --- | --- |
| 左右移动成功 | `comet-trail` | 在活动块后方短暂显示，可水平翻转复用 |
| 自然落地 / 硬降 | `glow-core` + `dust-impact` | 硬降提高尺寸和粒子数量，但不遮挡新活动块 |
| 方块沙化 | `noise-threshold` / `noise-cluster` + `sand-fall` / `dust-rise` + `grain-flow` | 每个宏格叠一层低透明像素阈值纹理，并用少量砂粒衔接现有固体淡出 |
| 横跨消除 | `clear-sweep` + `pulse-ring` + `dust-burst` | 按清除包围盒定位；连锁提高强调色和少量粒子数量 |
...
- `390×844` 浏览器复验通过：首页与棋盘布局不回退；移动拖尾、硬降落地和沙化反馈正确显示；干净会话无 error/warn。
